...
setwd("/rd/suth/2022/02March_Cancer_evo/Fig4a")
load("integrate_DE_info_result.rds")
load("integrate_DE_result.rds")

load("integrate_cox_result.rds")
names(integrate_cox_result)<-gsub("exp","uni",names(integrate_cox_result))

integrate_cox_result<-integrate_cox_result%>%select(gene,uni_p_value,uni_HR,uni_fdr,cancer_type)

integrate_cox_result<-integrate_cox_result%>%mutate(cox_class=ifelse(uni_HR>0&uni_fdr<=0.05,"up",ifelse(uni_HR<0&uni_fdr<=0.05,"down","no")))
...
pie_data <- pie_data |> filter(group != "intermediate")

pie_EM<-ggplot()+geom_bar(data=pie_data%>%filter(class=="EM"),aes(x="",y=prop,fill=group),stat="identity", size=0.1,width=1, color="white",alpha=.5)+coord_polar("y",start=0)+theme_void()+scale_fill_manual(values=c("violetred1","grey30",
#    "grey75",
    "palegreen3"))+theme(legend.text=element_text(size=15),legend.title=element_blank(),legend.position="none")

pie_PSG<-ggplot()+geom_bar(data=pie_data%>%filter(class=="PSG"),aes(x="",y=prop,fill=group),stat="identity", size=0.1,width=1, color="white",alpha=.5)+coord_polar("y",start=0)+theme_void()+scale_fill_manual(values=c("violetred1","grey30",
#    "grey75",
    "palegreen3"))+theme(legend.text=element_text(size=15),legend.title=element_blank(),legend.position="none")

pie_BG<-ggplot()+geom_bar(data=pie_data%>%filter(class=="BG"),aes(x="",y=prop,fill=group),stat="identity", size=0.1,width=1, color="white",alpha=.5)+coord_polar("y",start=0)+theme_void()+scale_fill_manual(values=c("violetred1","grey30",
#    "grey75",
    "palegreen3"))+theme(legend.text=element_text(size=15),legend.title=element_blank(),legend.position="none")

pie_UC<-ggplot()+geom_bar(data=pie_data%>%filter(class=="UC"),aes(x="",y=prop,fill=group),stat="identity", size=0.1,width=1, color="white",alpha=.5)+coord_polar("y",start=0)+theme_void()+scale_fill_manual(values=c("violetred1","grey30",
#    "grey75",
    "palegreen3"))+theme(legend.text=element_text(size=15),legend.title=element_blank(),legend.position="none")

pie_blank<-ggplot()

ggplot2::ggsave(file="fig4a.pdf",ggarrange(pie_blank,pie_EM,pie_UC,pie_PSG,nrow=2,ncol=2,widths=c(1,1),heights=c(1,1),draw=F),width=4,height=4,device="pdf")
